...
> The Security Fact contract is **LLD §23**, the Entity object **§24**, the
> Relationship object **§25**, and the normalized event schema **§21**.
> Where this document's schemas differ, the LLD wins. Two known gaps are
> escalated, not resolved: per-event fact granularity and the collector-side
> `risk_score` (ESC-4).
