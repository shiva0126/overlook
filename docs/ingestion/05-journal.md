...
> The journal is **NATS JetStream**, file-backed, `OVERLOOK_RAW` (LLD
> §14–16). There is no separate journal behind it — one durable write on the
> ingress path. Retention is escalated (ESC-1, ESC-2).
